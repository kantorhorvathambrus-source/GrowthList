...
## 9. "How did you hear about us" (`js/components/how-did-you-hear.js`)

Two placements, one Netlify form, identical field names.

```html
<section class="band band--alt hdyh" id="hdyh-footer|hdyh-prompt">
  <div class="rail"><span>One question</span></div>
  <div class="band-body">
    <div class="hdyh__head">
      <div><p class="eyebrow">Optional</p><h2 class="hdyh__heading">How did you find GrowthList?</h2></div>
      <button class="hdyh__dismiss" id="hdyh-dismiss">No thanks</button>  <!-- prompt only -->
    </div>
    <form name="how-did-you-hear" method="POST" data-netlify="true"
          netlify-honeypot="bot-field" class="hdyh__form" novalidate>
      <input type="hidden" name="form-name" value="how-did-you-hear">
      <p class="hdyh__hp" aria-hidden="true"><label>…<input name="bot-field"></label></p>
      <fieldset class="hdyh__fieldset">
        <legend class="hdyh__legend">How did you hear about GrowthList?</legend>
        <div class="hdyh__options">
          <label class="hdyh__opt" for="{prefix}-source-N">
            <input type="radio" id="{prefix}-source-N" name="source" value="Reddit"><span>Reddit</span>
          </label>  <!-- ×7 -->
        </div>
      </fieldset>
      <div class="hdyh__note"><label for="{prefix}-note">…</label>
        <input id="{prefix}-note" name="note" maxlength="200"></div>
      <div class="hdyh__actions">
        <button type="submit" class="button">Send</button>
        <span class="hdyh__status" role="status"></span>
      </div>
      <p class="hdyh__privacy">…</p>
    </form>
  </div>
</section>
```

- **The footer copy must stay static HTML in `index.html`.** Netlify's build
  bot scans deployed HTML to register a form; a JS-injected form is never
  detected. It is also the copy that works with JS disabled, as a plain POST.
  The prompt copy is generated by `formMarkup(prefix)` — **keep the two in
  sync on the name attributes** (`form-name`, `bot-field`, `source`, `note`).
  Ids differ by prefix so labels stay correctly associated; verified zero
  duplicate ids on the page.
- **Repeats:** 7 `.hdyh__opt` chips. They wrap to 3 rows at 390px.
- **States:**
  - `.hdyh__opt:has(input:checked)` — wine fill; the native radio is still
    visible inside the chip, so selection is never colour-alone.
  - `.hdyh__opt:has(input:focus-visible)` — the focus ring is drawn on the
    chip, since the radio itself is small.
  - `.hdyh__status` carries "Sending…", the validation nudge, and the failure
    message.
  - `.hdyh__thanks` replaces the whole form after a successful submit.
  - `.hdyh__hp` is the honeypot: positioned off-screen rather than
    `display: none`, which some bots skip.
- **Behaviour:** the prompt appears only after **3 distinct categories** in a
  session (`sessionStorage`), mounts **outside `#app`** so a view re-render
  cannot wipe it, and removes the footer copy while it is up so the same
  question is not asked twice on one page. Dismiss or submit writes
  `localStorage['growthlist:hdyh']` and both placements go for good.
- **Off Netlify the form is removed, not left to fail.** Detection is a
  same-origin `HEAD` probe for the `x-nf-request-id` header, cached per
  session — *not* a hostname check, because Netlify Dev serves from localhost.
