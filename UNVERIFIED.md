...
## Batch 64 — group A rounds 4–10: deferred, not rejected

Two candidates were set aside in these rounds without being written off.
Neither is in `probed.json` as a rejection, because the evidence against
them came from a window rather than the whole channel.

| Handle | What the API returned | Why it is deferred |
| --- | --- | --- |
| `@brianferoldiyt` | Brian Feroldi, 390 uploads, 100k–500k | **A second beginner tier for `stock-analysis`, nearly rejected on a window.** The 200 most recent uploads had 2 of 77 long-form on financial statements, which read as "not his subject". A full title scan of all 390 found 21 statement and ratio explainers of three minutes or more — separate "all the basics" videos on each statement, how the three link, the accounting equation for beginners. The Finance Storyteller was taken instead because its descriptions carry no affiliate notice and no funnel; Feroldi's carry a standing affiliate-commission notice and an ebook sign-up, and the channel is presented by two people. A real candidate if the category is funded past two. |
| `@barbelllogic` | Barbell Logic, 852 uploads, <100k | Round 7 (`strength-training`). 0 of 48 long-form in the recent 200 intersected lift terms with teaching framing once brand words were removed from the pattern — but titles such as "What to Do When Your Beginner Strength Program Stops Working" say beginner programming is its subject, and the channel was not full-scanned. Unjudged rather than rejected. |
| `@sean_nalewanyj` | Sean Nalewanyj, 681 uploads, 1M–5M | Round 10 (`hypertrophy-training`). The recent 200 uploads hold nothing of eight minutes or more, which nearly produced a rejection on the long-form floor — in a round whose band was chosen precisely because entry-tier explainers run four to twenty minutes. A full title scan found 21 beginner-framed videos of four minutes or more on eating to gain muscle, progressive overload and common mistakes, dated 2012–2022. Not evaluated for commerce or entry video; House of Hypertrophy was taken. Deferred. |
| `@artoftheproblem` | Art of the Problem, 174 uploads, 100k–500k | Funded list, `ai-fundamentals`. A real teacher of the concept — "What Is Machine Learning?", "What is Deep Learning?", "How Neural Networks Learn", clean descriptions — but a history-of-ideas channel whose AI teaching is 2019–2020 and whose recent long-form has moved to economics and philosophy essays; over half its uploads are shorts, and two recent sponsors are AI-model products. 3Blue1Brown's Deep Learning series was mapped instead under rule 15. Deferred as a possible third voice. |
| `@nirandfar` | Nir Eyal, 454 uploads, <100k | Found in `habit-formation`'s second round and **not** a fit there: most of his habit material is about designing habit-forming products (Hooked, habit testing for product teams), which is product design rather than building your own habits. His personal-behaviour material is attention and distraction — four long "Indistractable" talks (24–54m), timeboxing, precommitment pacts. That is a candidate for `deep-work-and-focus`, one of the six categories recorded as searched twice and not found. Recent uploads are 180 of 199 shorts; the long-form is 2012–2023. Not evaluated for commerce or entry video. Deferred to that category. |
| `ui-ux-design` slot | — | **The practising-designer slot is still open after three searches.** David Travis was added under the owner's option 1 as a research-method voice; he shares NNgroup's stance, so the category now has two views from one direction. Search 44 (designer end): career coaching (UX Anudeep), visual redesigns carrying a UI-pattern-library placement (Kole Jain), site-building tutorials. Search 45 (research end): testing-tool vendors, an accessibility vendor, archives. Search 46 (designer-process end): portfolio-building channels, and Mizko, whose research-led work is 8 of 152 long-form and whose channel has moved to founder content — recorded in `reputation-drift.json`. |
| `mortality-and-death` third voice | — | **Deferred, not rejected — three on-brief philosophy channels found alongside YaleCourses (funded list).** The Autodidact's Toolkit (`@autodidactstoolkit`): a 2016 set on the classic readings — Epicurus, Rosenbaum, Pitcher, Williams on immortality, Feldman — 36–51m each. Reason Amidst Ruin (`@reasonamidstruin`): Socrates, Plato and Epicurus on death, 2020, 27–37m. Simon Cushing (`@simoncushing`): "When is death bad for you?" (63m) and bioethics lectures. None evaluated for commerce or entry video; YaleCourses' Death course was taken as the most complete. |
| `@cristibundukamara` | Mentally STRONG by Dr. B, 412 uploads, <100k | **HELD — excluded from `grief-and-loss` under rule 12 for commercial conflict. The owner's decision; not a candidate.** The teaching is on brief (a self-described psychiatric nurse practitioner; "Grief theories \| 5 stages and more" debunks the stage model and covers the dual process model, continuing bonds and prolonged grief disorder). The conflict is the reason: 217 of 412 descriptions sell programmes, including a paid "Grief Intensive", to people who are actively grieving — in a category whose own notFor says education, not support. This is NOT the `sells-course` pattern of selling unrelated products to a general audience; it is a sale made to the audience rule 12 exists to protect, at the moment it is least able to judge it. |
| `@irishhospice` | Irish Hospice Foundation, 269 uploads | Deferred as a possible third `grief-and-loss` voice: the national bereavement charity's public information evenings (90–105m) and a 30-minute "Understanding Grief and Loss". Not evaluated for entry video. |

- **Nash Consulting (`@nashconsulting4887`) — deferred, `people-management` third voice (batch 64).** A management-training firm since 1996; 189 of 198 uploads are long-form (median 39m) and 11 of them match subject-plus-teaching framing, including one-on-ones and a named feedback method. Not taken as the second creator because the densities are close (subject grep: Nash 33 of 189 long-form, Ramona Shaw 23 of 109; subject-plus-teaching: 11 of 189 against 8 of 109) and the difference is format: 4 of Nash's 11 matches name a guest in the title, while 1 of Ramona Shaw's 34 recent long-form uploads does, and her matches are all from 2024 on against Nash's spread back to 2019. Worth a full read if the category is funded to depth 3. `entity` would be institution.

- **Karin Prater (`@swiftyplace`) — deferred, `mobile-development` comparable active alternative (batch 64).** Active; 117 of 129 long-form uploads on iOS/SwiftUI, median 34m, and 49 of 129 framed as tutorials or courses. Not taken because Paul Hudson's numbered project curriculum matches the beginner brief (build one small app end to end) more directly. First candidate if the category is funded to depth 3. **iOS Insights (`@ios-insights`)** also deferred, unexamined beyond density: 185 of 185 long-form on subject, 107 framed as teaching.
- **`mobile-development` shipping-and-release voice — still OPEN (batch 64).** Six channels scanned for app store / TestFlight / release / publish / submit / ship / app review / launch in titles: Paul Hudson 2 of 89 long-form, Karin Prater 4 of 129, CodeWithChris 5 of 133, iOS Insights 0 of 185, DesignCode 0 of 131, azamsharp 0 of 112. Hudson's one long App Store video is a walkthrough of his own paid-tier tool (rule 24 addendum). Probed from the iOS end only — the release-engineering end (fastlane, CI for mobile, Play Console) was not searched, so this is a report about this search, not about supply (rule 18).

- **The Coffee Chronicler (`@coffeechronicler`) — deferred, `coffee-brewing` depth-3 candidate (batch 64).** Asser Christensen, a licensed Q Grader per his own channel description; active; 55 of 104 long-form uploads on brewing subjects, but only 5 framed as teaching a variable or method against 40 framed as reviews or comparisons. Lance Hedrick was taken on teaching density (21 of 193, with a structured one-variable series). Read in full before any depth-3 pick.

- **Two Sides of FI (`@twosidesoffi`) — deferred, `financial-independence` depth-3 candidate for the advanced level (batch 64).** US; active; two friends who reached FI in 2020 and 2024 talking about life after it — 78 of 131 long-form uploads on FI terms, 22 touching mechanics (paying yourself, healthcare, spending). Lived experience rather than modelling, which is the brief's advanced tier (identity and purpose after the number), not its intermediate one. Two guest episodes centre on options-trading windfalls; read those before any pick. **Rob Berger** (already listed, `retirement-planning`) was NOT mapped here: his withdrawal-plan videos are the evidence for his existing mapping, so a second mapping would count one body of work twice (rule 4).

- **Jennifer May, Ph.D. (`@jennifermayph.d.2761`) — deferred, `setting-boundaries` (batch 64).** A licensed clinical psychologist and DBT coordinator per her channel description; **dormant** (last upload 2025-02). The densest on-brief catalogue found: 19 of 200 long-form on boundaries, assertiveness and saying no, including the full DBT interpersonal-effectiveness module as 11–22 minute lessons ("Saying 'No'", DEAR MAN, FAST, GIVE) and RO-DBT lessons on pushback responses. Not taken under rule 25: DBT & Me Podcast is active, in the same frame, and on the same skills — comparable in quality, different in format (hour-long conversation against short lessons). Her shorter lessons would suit the beginner level better; the natural depth-3 pick, listed as inactive with the caveat rule 25 requires.
  **Corrected (batch 65, owner's decision): she is now LISTED, and the reasoning above was wrong.** "Comparable in quality, different in format" treated format as separate from quality; for a beginner/intermediate slot it is part of it. Measured over both whole catalogues with one pattern: 17 of her 218 long-form against 7 of DBT & Me's 187, lessons of 11–22 minutes against episodes of 59–102. Rule 25 lets an active candidate win only at comparable quality, so activity decided this pick wrongly. She is listed as dormant with the rule 25 caveat (entry: "Saying No", 16 min); DBT & Me stays, its entry moved from the 102-minute boundary episode to "Episode 33 - FAST" (59 min), chosen from its description only.
- **Doc Snipes (already listed, `emotional-regulation`) — NOT mapped to `setting-boundaries` (batch 64).** Two 2026 lectures on assertiveness and healthy boundaries are different videos from her existing mapping's evidence, so rule 4 would not block it; rule 15's trigger would honestly be "filling this category", and an active new creator was available.

- **Ditto Insurance (`@dittoinsurance`) — deferred, `insurance-basics` (batch 65).** An Indian insurance advisory platform that sells health and term policies through its own advisers, per its channel description; active; 151 of 200 recent uploads on insurance, 20 of 28 long-form. Not evaluated past subject density: upload language, teaching framing and a rule 24 read are all still to do. If taken it is `commercial-conflict` (it earns when a viewer buys) and `jurisdiction: IN`; rule 13's non-profit test would decide on content.

- **MeaningfulMoney (`@meaningfulmoney`) — deferred pending the owner's decision, `insurance-basics` UK slot (batch 65).** Pete Matthew, per the channel description; GB; active; 713 uploads. Full-catalogue title scan: 7 of 323 long-form on insurance or protection. **Rule 24 blocks the recent ones:** LifeSearch, a protection-insurance broker, appears in 217 of 713 descriptions, with "a special deal that I have with LifeSearch" through a vanity path on the creator's own domain (`meaningfulmoney.tv/lifesearch`); "The Ultimate Guide To WEALTH PROTECTION" (36m, 2020) has a "Call LifeSearch" chapter, and the 2019 income-protection episode, the 2020 COVID episode and the 2021 update are interviews with LifeSearch's founder. **The clean ones are 2010–2011:** "MM No. 15 - Term Life Assurance" (9m), "MM No. 20 - Income Protection Insurance" (8m), "Episode 84 - Basics: Insurance" (8m) — no placement in their descriptions, but fifteen years old on a channel whose own description says old videos may need updating. If taken: `commercial-conflict`, `jurisdiction: UK`. **Owner's decision (batch 65): DECLINED** — a fifteen-year-old entry video, on a channel that warns its own old videos may be out of date, is not a basis for a money category. Not a fallback. The slot went to Skye Wealth (AU) on a third search angle.
